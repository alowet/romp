...
test("the composer closure's call sites: deliver sends only through flushStaged with the typed message, the stage path is the one reveal, and nothing collapses a list on its own", () => {
  ...
  assert.match(RENDER, /import \{ StagedStack, quoteReplyBody, stagedPosts, type StagedMsg, isCitationShape \} from "\.\/staged-messages";/);   // + the type: a moved tab's staged messages travel with it (the chat split, 2026-09-11); + the one citation shape check (round eleven)
