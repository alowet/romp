...
test("closing a provisional tab aborts the pending spawn; a FAILED one is a plain local discard", () => {
  assert.match(RENDER, /if \(id === provisionalId\) cancelProvisional\(\);\s*\n\s*else \{ failedProvisionals\.delete\(id\); failedWhy\.delete\(id\); failedInfo\.delete\(id\); stagedMsgs\.takeAll\(id\); dismissSession\(id, "close"\); syncColumnBusy\(\); \}/);   // …its reason goes with it (round five)   // …and the shell hears the column is no longer busy (a held drop of it applies; chat-split.test.ts)
