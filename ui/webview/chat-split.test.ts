...
test("a pick of a session another column holds is shown where it lives: the setActive guard precedes the peek and the drafts swap", () => {
  ...
  assert.match(RENDER, /function columnBusy\(\): boolean \{ return !!provisionalId \|\| failedProvisionals\.size > 0 \|\| pendingShips\.size > 0 \|\| unverifiedHere\(\); \}\n\(window as any\)\.__rompColumnBusy = \(\): boolean => columnBusy\(\);/);
  ...
  assert.match(RENDER, /function orphanStateSids\(\): string\[\] \{[\s\S]*?if \(!isProvisionalId\(id\) && !isSubId\(id\) && !heldHere\(id\) && !isUnverified\(id\)\) out\.add\(id\);/);
  ...
  // picker opened over the prompt dismisses it first, as its own create's
  ...
  assert.match(move, /composerFiles\.set\(toId[\s\S]*stagedMsgs\.appendAll\(toId, staged\);[\s\S]*mergeCitations\(toId, cites\);[\s\S]*pendingShips\.set\(toId[\s\S]*if \(sendOnShip\.delete\(fromId\)\) armHold\(toId\);\n  if \(shipGateSid === fromId\) \{ if \(isUnverified\(toId\)\) \{ shipGateSid = null; closeConfirm\(null\); warnToast\(UNVERIFIED_NOTICE\); \} else shipGateSid = toId; \}\n\}/, "the move: files (their marks with them), stack, chips, then the upload's chip; the held send and the gate through the one door (round twenty-one)");
  // round twenty-one: the durable mark and its one door
  assert.match(RENDER, /interface ComposerFile \{ id: string; path: string; legacy: boolean; why\?: "store" \}[^\n]*\nconst composerFiles = new Map<string, ComposerFile\[\]>\(\);/, "the mark, a stable id and the mark\'s reason live on the entry (rounds twenty-two to twenty-four)");
  assert.match(RENDER, /const files = \(composerFiles\.get\(sid\) \|\| \[\]\)\.filter\(\(e\) => !armedF\.has\(e\.id\)\);[^\n]*\n  for \(const e of stash\.files\) if \(!files\.some\(\(x\) => x\.id === e\.id\)\) files\.push\(\{ \.\.\.e \}\);/, "the rollback reconciles by id, never by path");
  assert.match(RENDER, /function fileWire\(list: readonly ComposerFile\[\]\): \{ files: string\[\]; filesLegacy: boolean\[\]; filesIds: string\[\] \}/, "the wire/store view: the old shape plus aligned siblings");
  assert.match(RENDER, /function isUnverified\(sid: string \| null\): boolean \{ return !!sid && \(composerFiles\.get\(sid\) \|\| \[\]\)\.some\(\(e\) => e\.legacy\); \}/);
  assert.ok(!RENDER.includes("legacyFiles") && !RENDER.includes("markLegacyFile") && !RENDER.includes("dropLegacyMarks") && !RENDER.includes("moveLegacyMarks"), "the parallel mark store and its helpers are gone");
  assert.match(RENDER, /function restoreComposerAfterRefusedRescind\(sid: string, stash: \{ cites: Citation\[\]; files: ComposerFile\[\]; armedCites: string\[\]; armedFiles: string\[\] \}\): void \{[\s\S]{0,900}files\.push\(\{ \.\.\.e \}\);/, "the rollback restores entries with their provenance");
  assert.match(RENDER, /function armHold\(sid: string\): boolean \{\n  if \(isUnverified\(sid\)\) \{ warnToast\(UNVERIFIED_NOTICE\); return false; \}\n  sendOnShip\.add\(sid\);\n  return true;\n\}/);
  assert.equal((RENDER.match(/sendOnShip\.add\(/g) || []).length, 1, "one door: sendOnShip.add lives in armHold alone");
  assert.match(RENDER, /addComposerFile\(owner, m\.path, !tagged\);/, "an untagged answer's attach marks the composer"); assert.match(RENDER, /if \(isUnverified\(retired\)\) \{ disarmUnverified\(retired\); \}/, "the release belt"); assert.match(RENDER, /if \(isUnverified\(sid\)\) \{ warnToast\(UNVERIFIED_NOTICE\); return; \}/, "sendHeldFor's belt");
  assert.match(RENDER, /if \(isUnverified\(sid\)\) \{\n(?:\s*\/\/[^\n]*\n)*\s*showConfirm\(what \+ " still uploading",\n[\s\S]{0,400}\[\{ label: "Send without " \+ them, value: "now", danger: true \}\],/, "the gate offers no Wait on an unverified composer");
  assert.match(RENDER, /files: Object\.fromEntries\(\[\.\.\.composerFiles\]\.map\(\(\[k, v\]\) => \[k, fileWire\(v\)\.files\]\)\),\n\s*filesMeta: Object\.fromEntries\(\[\.\.\.composerFiles\]\.map\(\(\[k, v\]\) => \[k, fileMetaOf\(v\)\]\)\),/, "persisted in the old shape, the marks in a record bound to its own copy of the paths (round twenty-four)");
  assert.match(RENDER, /if \(m && typeof m === "object" && sameStrings\(\(m as any\)\.files, v\)\) entries = fileEntriesOf\(v, \(m as any\)\.legacy, \(m as any\)\.ids, \(m as any\)\.why\);/, "the record applies only when it names exactly these paths");
  assert.match(RENDER, /entries = fileEntriesOf\(v\)\.map\(\(e\) => \(\{ \.\.\.e, legacy: true, why: "store" as const \}\)\); rebind = true;/, "a stale record: every entry unverified, the store rewritten");
  assert.match(RENDER, /delete prior\.filesLegacy; delete prior\.filesIds;/, "the interim unbound maps leave the store"); assert.ok(!/filesLegacy: Object\.fromEntries/.test(RENDER) && !/filesIds: Object\.fromEntries/.test(RENDER)); assert.match(RENDER, /entries = fileEntriesOf\(v, \(m as any\)\.legacy, \(m as any\)\.ids, \(m as any\)\.why\);/, "…and rebuilt from the bound record; an older store's bare paths load as verified entries (round twenty-four)"); assert.match(RENDER, /if \(isUnverified\(sid\)\) disarmUnverified\(sid\);/, "adoptSessionState: a hold on the receiving session is cancelled");
  ...
  assert.match(RENDER, /function orphanStateSids\(\): string\[\] \{[\s\S]*?\[\.\.\.drafts\.keys\(\), \.\.\.composerCitations\.keys\(\), \.\.\.composerFiles\.keys\(\), \.\.\.Object\.keys\(stagedMsgs\.entries\(\)\)\][\s\S]*?if \(!isProvisionalId\(id\) && !isSubId\(id\) && !heldHere\(id\) && !isUnverified\(id\)\) out\.add\(id\);/, "the orphan offer never carries an unverified state out of its document (round twenty-four)");
...
test("drafts travel with a moved tab: the source hands over what it holds, synchronously, and the target adopts it", () => {
  ...
  assert.match(take, /const draft = drafts\.get\(sid\) \?\? "", citations = composerCitations\.get\(sid\) \?\? \[\], wire = fileWire\(composerFiles\.get\(sid\) \?\? \[\]\), files = wire\.files, staged = stagedMsgs\.takeAll\(sid\);/);
  ...
  assert.match(take, /if \(!draft && !citations\.length && !files\.length && !staged\.length\) return null;\n  return \{ draft, citations, files, staged, filesLegacy: wire\.filesLegacy, filesIds: wire\.filesIds \};/, "null when nothing was held; the four old fields exactly as they were, the provenance and the ids beside them (round twenty-three)");
  assert.match(take, /wire = fileWire\(composerFiles\.get\(sid\) \?\? \[\]\), files = wire\.files,/, "the files travel as paths: an older reader in the same dashboard keeps them");
  ...
  assert.match(RENDER, /function columnBusy\(\): boolean \{ return !!provisionalId \|\| failedProvisionals\.size > 0 \|\| pendingShips\.size > 0 \|\| unverifiedHere\(\); \}/, "four facts, one reader (round twenty-four: the unverified attachment)");
  ...
  assert.match(RENDER, /\(window as any\)\.__rompSessionBusy = \(sid: unknown\): boolean => typeof sid === "string" && !!sid && \(pendingShips\.has\(sid\) \|\| sendOnShip\.has\(sid\) \|\| isUnverified\(sid\)\);/, "an unverified attachment pins its session (round twenty-four)");
  assert.match(RENDER, /\(window as any\)\.__rompSessionBusyWhy = \(sid: unknown\): "" \| "upload" \| "unverified" => typeof sid !== "string" \|\| !sid \? "" : isUnverified\(sid\) \? "unverified" : \(pendingShips\.has\(sid\) \|\| sendOnShip\.has\(sid\)\) \? "upload" : "";/);
  assert.match(RENDER, /function columnBusy\(\): boolean \{ return !!provisionalId \|\| failedProvisionals\.size > 0 \|\| pendingShips\.size > 0 \|\| unverifiedHere\(\); \}/, "the column's fourth fact");
  assert.match(RENDER, /function columnBusyWhy\(\): "" \| "create" \| "failed" \| "upload" \| "upload-unshown" \| "unverified" \{[\s\S]{0,400}return unverifiedHere\(\) \? "unverified" : "";/);
  assert.match(RENDER, /if \(m\.romp === "askCloseUnverified"\) \{ askCloseUnverified\(\); return; \}/); assert.match(RENDER, /\(v\) => \{ if \(v === "close"\) \{ discardUnverified\(\); try \{ \(window\.parent as any\)\?\.__rompCloseSplit\?\.\(Number\(COL\)\); \}/, "\"Close anyway\" discards the unverified chips, then closes");
  assert.ok(KERNEL.includes("if(bw==='unverified'){try{f.contentWindow.postMessage({romp:'askCloseUnverified'},'*');}catch(e){}return;}"), "the shell's close asks for the unverified hold");
  assert.ok(KERNEL.includes("var sw=sessionBusyAnywhere(sid);if(sw)return notify(sw==='unverified'?UNVERIFIED:UPLOADING);"), "the shell's move names the fact");
  assert.ok(KERNEL.includes("sessionBusyAnywhere(sid)") && KERNEL.includes("notify(sw==='unverified'?UNVERIFIED:UPLOADING)"), "moveTab asks every column document per session (round thirteen), and names the fact (round twenty-four)");
  assert.ok(KERNEL.includes("if(!keep&&busy(f)){var bw=busyWhy(f);if(bw==='upload-unshown'){try{f.contentWindow.postMessage({romp:'askCloseUpload'},'*');}catch(e){}return;}"), "the close asks the page when an unshown session's upload is the only hold");
  ...
  assert.match(RENDER, /function composerShips\(sid: string \| null\): PendingShip\[\] \{/); assert.match(RENDER, /if \(retired && tagged && \(sendOnShip\.has\(retired\) \|\| gateOpen\) && !composerShips\(retired\)\.length\) \{/, "the composer's own ships release a held send — on a TAGGED answer (round nineteen)");
  assert.match(RENDER, /const shipping = composerShips\(activeId\)\.length;/); assert.match(RENDER, /if \(id && !composerShips\(id\)\.length\) \{/);
  assert.match(RENDER, /function reclaimHandedOff\(\): void \{ for \(const sid of \[\.\.\.handedOff\]\) if \(heldHere\(sid\)\) handedOff\.delete\(sid\); \}/); assert.match(RENDER, /colSets = readColSets\(\);[^\n]*\n  reclaimHandedOff\(\);/, "the sid shown again clears the marker where the members are read");
  assert.ok(KERNEL.includes("function sessionBusyAnywhere(sid){var fs=frames();for(var i=0;i<fs.length;i++){var w=sessionBusyWhy(fs[i],sid);if(w)return w;}return '';}") && KERNEL.includes("var sw=sessionBusyAnywhere(sid);if(sw)return notify(sw==='unverified'?UNVERIFIED:UPLOADING);"), "the move guard asks every column document");
  ...
  // round nineteen: an untagged answer has no send side effects — an exact-name best guess for the FILE among the answering host's posted
  // ships, first posted first; its owner's hold is cancelled and said; the ambiguity machinery is gone; tagged answers settle as at 5818185b
  assert.match(RENDER, /function legacyNameOf\(path: string\): string \{ return \(path\.split\("\/"\)\.pop\(\) \|\| path\)\.replace\(\/\^\\d\+-\/, ""\); \}/);
  assert.match(RENDER, /function legacyShipFor\(name: string, host: string\): PendingShip \| null \{\n  const want = shipSafeName\(name\);\n  let best: PendingShip \| null = null;\n  for \(const \[sid, list\] of pendingShips\) \{\n    if \(hostOf\(sid\) !== host\) continue;\n    for \(const p of list\) if \(p\.postSeq !== undefined && shipSafeName\(p\.name\) === want && \(!best \|\| p\.postSeq < best\.postSeq!\)\) best = p;\n  \}\n  return best;\n\}/, "exact name, the answering host's posted ships, the first posted");
  assert.ok(!RENDER.includes("noteAmbiguity") && !RENDER.includes("ambiguous"), "the ambiguity machinery is gone");
  assert.match(RENDER, /function postShipFrame\(sid: string \| null, p: PendingShip\): void \{[\s\S]{0,400}p\.postSeq = \+\+postOrder;\n  vscodeApi\.postMessage\(msg\);\n\}/);
  assert.equal((RENDER.match(/postShipFrame\(/g) || []).length, 3, "defined once, called from the reader's post and the re-ship"); assert.equal((RENDER.match(/\{ type: "dropFile", name/g) || []).length, 1, "one place builds the frame");
  assert.match(RENDER, /const tagged = !!ackShip;[^\n]*\n    if \(!ackShip && !picked\) \{ const legacy = legacyShipFor\(legacyNameOf\(m\.path\), typeof m\.host === "string" \? m\.host : ""\); if \(legacy\) ackShip = legacy\.shipId; \}/);
  assert.match(RENDER, /const gateOpen = shipGateSid === owner;\n    if \(!tagged && \(sendOnShip\.has\(owner\) \|\| gateOpen\)\) \{[\s\S]{0,700}answered by an older kernel that names no upload, so the message was not sent automatically[\s\S]{0,400}arrived from an older kernel that names no upload and was attached here[\s\S]{0,300}\} else if \(retired && tagged && \(sendOnShip\.has\(retired\) \|\| gateOpen\) && !composerShips\(retired\)\.length\) \{/, "untagged: whichever composer the file lands on — matched or the active fallback — has its send disarmed and said (round twenty); tagged: released as at 5818185b");
  assert.match(FED, /if \(out\.type === "droppedPath" \|\| out\.type === "dropSaveFailed"\) out\.host = host;/, "federation stamps a remote kernel's upload answers with the answering host");
  assert.match(RENDER, /list\.push\(\{ name, shipId, kind \}\);/);
  assert.match(RENDER, /const picked = m\.picked === true;[^\n]*\n    let ackShip = [^\n]*\n    if \(ackShip && !shipOwner\(ackShip\)\) return;/);
  ...
  assert.match(RENDER, /if \(!nackShip\) \{ const legacy = legacyShipFor\(m\.name, typeof m\.host === "string" \? m\.host : ""\); if \(legacy\) nackShip = legacy\.shipId; \}/);
  assert.match(RENDER, /function shipFailed\(key: string, shipId: string \| undefined, why: string\): void \{\n(?:\s*\/\/[^\n]*\n)*\s*if \(!shipId\) \{ warnToast\(why\); return; \}/);
  ...
  assert.match(RENDER, /const retired = retirePendingShip\(m\.path, ackShip\);[^\n]*\n    const owner = retired \|\| activeId;[^\n]*\n    if \(!owner\) \{ warnToast\([^\n]*\n    addComposerFile\(owner, m\.path, !tagged\);/); assert.match(RENDER, /if \(retired && tagged && \(sendOnShip\.has\(retired\) \|\| gateOpen\) && !composerShips\(retired\)\.length\) \{/, "a held send fires only from a TAGGED ack that retired its owner's composer ship (rounds fifteen, nineteen)");
  ...
  // round thirteen REVERTS two round-twelve additions, pinned by their absence: the shell's parent-side tryFire (it fired while the moved
  // state lived only in make()'s load listener or a queued postMessage) and the kernel's (sid, shipId) save cache (a page-local id, a
  // kernel-global key: another document's different bytes answered with the first upload's path). The page's deferred ended() and its
  // shim-queue skip stand.
  assert.ok(!KERNEL.includes("reloadEnded") && !KERNEL.includes("_saved_drop_path") && !KERNEL.includes("_DROP_SAVED"), "both reverted");
  assert.ok(KERNEL.includes('fp = _save_dropped_file(str(msg["name"]), str(msg["b64"]))   # bytes → saved file → insert its path'), "the dropFile branch as on main");
  assert.match(RENDER, /if \(!hosts && p\.queued\) \{ p\.queued = false; continue; \}/, "…the page-side skip stays");
