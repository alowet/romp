...
test("drafts travel with a moved tab: the source hands over what it holds, synchronously, and the target adopts it", () => {
  ...
  assert.match(RENDER, /if \(retired && ambiguous && \(sendOnShip\.has\(retired\) \|\| gateOpen\)\) \{/, "an ambiguous legacy match never auto-sends"); assert.match(RENDER, /list\.push\(\{ name, shipId, kind, seq: \+\+shipOrder \}\);/);
