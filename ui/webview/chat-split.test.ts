...
test("a pick of a session another column holds is shown where it lives: the setActive guard precedes the peek and the drafts swap", () => {
  ...
  assert.match(RENDER, /function orphanStateSids\(\): string\[\] \{[\s\S]*?if \(!isProvisionalId\(id\) && !isSubId\(id\) && !heldHere\(id\)\) out\.add\(id\);/);
  ...
  assert.match(RENDER, /interface ComposerFile \{ id: string; path: string; legacy: boolean \}\nconst composerFiles = new Map<string, ComposerFile\[\]>\(\);/, "the mark and a stable id live on the entry (rounds twenty-two, twenty-three)");
  ...
  assert.match(RENDER, /files: Object\.fromEntries\(\[\.\.\.composerFiles\]\.map\(\(\[k, v\]\) => \[k, fileWire\(v\)\.files\]\)\),\n\s*filesLegacy: /, "persisted in the old shape with the siblings beside it"); assert.match(RENDER, /const entries = fileEntriesOf\(v, /, "…and rebuilt from the three lists; an older store's bare paths load as verified entries"); assert.match(RENDER, /if \(isUnverified\(sid\)\) disarmUnverified\(sid\);/, "adoptSessionState: a hold on the receiving session is cancelled");
