...
test("drafts travel with a moved tab: the source hands over what it holds, synchronously, and the target adopts it", () => {
  ...
  assert.match(RENDER, /function legacyShipFor\(key: string, host: string\): \{ ship: PendingShip; matches: number \} \| null \{\n  const k = "-" \+ shipSafeName\(key\.split\("\/"\)\.pop\(\) \|\| key\);\n  const found: PendingShip\[\] = \[\];\n  for \(const \[sid, list\] of pendingShips\) \{ if \(hostOf\(sid\) !== host\) continue; for \(const p of list\) if \(p\.postSeq !== undefined && k\.endsWith\("-" \+ shipSafeName\(p\.name\)\)\) found\.push\(p\); \}\n  if \(!found\.length\) return null;\n  found\.sort\(\(a, b\) => a\.postSeq! - b\.postSeq!\);\n  return \{ ship: found\[0\], matches: found\.length \};\n\}/, "a legacy answer: the answering host's POSTED ships alone, the first posted, and how many could be it (rounds seventeen, eighteen)");
  // round eighteen: one post site stamps the order and checks the name; the reader and every re-ship go through it; a retired ambiguous ship never releases
  ...
  assert.match(RENDER, /const retiredShip = ackShip \? shipRecord\(ackShip\) : null;[^\n]*\n    if \(retiredShip && retiredShip\.ambiguous\) ambiguous = true;/);
  assert.match(RENDER, /function noteAmbiguity\(sid: string \| null, p: PendingShip\): void \{[\s\S]{0,700}q\.ambiguous = true;[\s\S]{0,600}is on its way for a different session, so this message will not be sent automatically/);
  ...
  assert.match(RENDER, /if \(retired && ambiguous && \(sendOnShip\.has\(retired\) \|\| gateOpen\)\) \{/, "an ambiguous legacy match never auto-sends"); assert.match(RENDER, /list\.push\(\{ name, shipId, kind \}\);/);
  assert.match(RENDER, /const picked = m\.picked === true;[^\n]*\n    let ackShip = [^\n]*\n    if \(ackShip && !shipOwner\(ackShip\)\) return;[\s\S]{0,500}if \(!ackShip && !picked\) \{ const legacy = legacyShipFor\(m\.path, typeof m\.host === "string" \? m\.host : ""\); if \(legacy\) \{ ackShip = legacy\.ship\.shipId; ambiguous = legacy\.matches > 1; \} \}/);
  ...
  assert.match(RENDER, /if \(!nackShip\) \{ const legacy = legacyShipFor\(m\.name, typeof m\.host === "string" \? m\.host : ""\); if \(legacy\) nackShip = legacy\.ship\.shipId; \}/);
  assert.match(RENDER, /function shipFailed\(key: string, shipId: string \| undefined, why: string\): void \{\n(?:\s*\/\/[^\n]*\n)*\s*if \(!shipId\) \{ warnToast\(why\); return; \}/);
