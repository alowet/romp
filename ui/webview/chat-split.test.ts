...
test("the paths: every write of activeId records the intent, the automatic paths are enumerated, the shell's fill says `auto`, and the kernel reads the flag", () => {
  ...
  assert.match(RENDER, /if \(m\.auto === true\) withAuto\(land\); else land\(\);/, "a focus message is a navigation unless its sender said `auto`");
  ...
  // the two re-announcements of where the user is
