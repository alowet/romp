...
test("drafts travel with a moved tab: the source hands over what it holds, synchronously, and the target adopts it", () => {
  ...
  assert.match(take, /if \(!draft && !citations\.length && !files\.length && !staged\.length\) return null;\n  return \{ draft, citations, files, staged, \.\.\.\(disarm \? \{ disarm: true as const \} : \{\}\) \};/, "null when nothing was held; four fields, exactly");
  ...
  assert.match(RENDER, /if \(retired === activeId\) fireHeldSend\(\);\n      else sendHeldFor\(retired\);[^\n]*\n      endReloadHoldIfIdle\(\);[^\n]*\n    \}\n    if \(!tagged && owner && !heldHere\(owner\)\) disarmOnTake\.add\(owner\);[^\n]*\n    if \(owner && !heldHere\(owner\)\) noteOrphanState\(\);[^\n]*\n    syncColumnBusy\(\);[^\n]*\n  \} else if \(m\.type === "dropSaveFailed"/, "the ack: the held send fires by sid when the tab is not shown; a completed file under an unshown sid is offered — an untagged answer's guess with a one-shot disarm (round twenty-six); the flip last");
  ...
  assert.match(RENDER, /const gateOpen = shipGateSid === owner;\n    if \(!tagged && \(sendOnShip\.has\(owner\) \|\| gateOpen\)\) \{[\s\S]{0,700}answered by an older kernel that names no upload, so the message was not sent automatically[\s\S]{0,400}: UNTAGGED_ATTACHED_NOTICE\);[\s\S]{0,300}\} else if \(retired && tagged && \(sendOnShip\.has\(retired\) \|\| gateOpen\) && !composerShips\(retired\)\.length\) \{/, "untagged: whichever composer the file lands on — matched or the active fallback — has its send disarmed and said (round twenty); tagged: released as at 5818185b");
  assert.match(RENDER, /const UNTAGGED_ATTACHED_NOTICE = "A file arrived from an older kernel that names no upload and was attached here, so this message will not be sent automatically — check the attachment and send it yourself\.";/, "the one notice, shared with the hand-off's disarm (round twenty-six)");
  ...
  assert.match(RENDER, /const retired = retirePendingShip\(m\.path, ackShip\);[^\n]*\n    const owner = retired \|\| activeId;[^\n]*\n    if \(!owner\) \{ warnToast\([^\n]*\n    addComposerFile\(owner, m\.path\);/); assert.match(RENDER, /if \(retired && tagged && \(sendOnShip\.has\(retired\) \|\| gateOpen\) && !composerShips\(retired\)\.length\) \{/, "a held send fires only from a TAGGED ack that retired its owner's composer ship (rounds fifteen, nineteen)");
