...
test("a pick of a session another column holds is shown where it lives: the setActive guard precedes the peek and the drafts swap", () => {
  ...
  assert.match(RENDER, /interface ComposerFile \{ path: string; legacy: boolean \}\nconst composerFiles = new Map<string, ComposerFile\[\]>\(\);/, "the mark lives on the entry (round twenty-two)");
  ...
  assert.match(RENDER, /files: Object\.fromEntries\(composerFiles\),/, "persisted as entries with the drafts"); assert.match(RENDER, /const entries = fileEntriesOf\(v\);/, "…and an older store's bare paths load as verified entries"); assert.match(RENDER, /if \(isUnverified\(sid\)\) disarmUnverified\(sid\);/, "adoptSessionState: a hold on the receiving session is cancelled");
...
test("drafts travel with a moved tab: the source hands over what it holds, synchronously, and the target adopts it", () => {
  ...
  assert.match(take, /if \(!draft && !citations\.length && !files\.length && !staged\.length\) return null;\n  return \{ draft, citations, files, staged \};/, "null when nothing was held; four fields, exactly — the files are entries that carry their provenance (round twenty-two)");
