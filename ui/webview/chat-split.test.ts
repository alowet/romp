...
test("a pick of a session another column holds is shown where it lives: the setActive guard precedes the peek and the drafts swap", () => {
  ...
  assert.match(move, /composerFiles\.set\(toId[\s\S]*moveLegacyMarks\(fromId, toId\);[\s\S]*stagedMsgs\.appendAll\(toId, staged\);[\s\S]*mergeCitations\(toId, cites\);[\s\S]*pendingShips\.set\(toId[\s\S]*if \(sendOnShip\.delete\(fromId\)\) armHold\(toId\);\n  if \(shipGateSid === fromId\) \{ if \(isUnverified\(toId\)\) \{ shipGateSid = null; closeConfirm\(null\); warnToast\(UNVERIFIED_NOTICE\); \} else shipGateSid = toId; \}\n\}/, "the move: files (their marks with them), stack, chips, then the upload's chip; the held send and the gate through the one door (round twenty-one)");
  // round twenty-one: the durable mark and its one door
  assert.match(RENDER, /function isUnverified\(sid: string \| null\): boolean \{ return !!sid && \(legacyFiles\.get\(sid\)\?\.size \?\? 0\) > 0; \}/);
  ...
  assert.match(RENDER, /legacy: Object\.fromEntries\(\[\.\.\.legacyFiles\]\.map\(\(\[k, v\]\) => \[k, \[\.\.\.v\]\]\)\),/, "persisted with the drafts"); assert.match(RENDER, /if \(isUnverified\(sid\)\) disarmUnverified\(sid\);/, "adoptSessionState: a hold on the receiving session is cancelled");
...
test("drafts travel with a moved tab: the source hands over what it holds, synchronously, and the target adopts it", () => {
  ...
  assert.match(take, /drafts\.delete\(sid\); composerCitations\.delete\(sid\); composerFiles\.delete\(sid\); legacyFiles\.delete\(sid\);\n/);
  ...
  assert.match(take, /if \(!draft && !citations\.length && !files\.length && !staged\.length\) return null;\n  return \{ draft, citations, files, staged, legacy \};/, "null when nothing was held; five fields, exactly — the unverified marks ride with their files (round twenty-one)");
