...
test("drafts travel with a moved tab: the source hands over what it holds, synchronously, and the target adopts it", () => {
  ...
  assert.match(RENDER, /if \(retired && !tagged && \(sendOnShip\.has\(retired\) \|\| gateOpen\)\) \{[\s\S]{0,700}answered by an older kernel that names no upload, so the message was not sent automatically[\s\S]{0,300}\} else if \(retired && tagged && \(sendOnShip\.has\(retired\) \|\| gateOpen\) && !composerShips\(retired\)\.length\) \{/, "untagged: the hold is cancelled and said; tagged: released as at 5818185b");
