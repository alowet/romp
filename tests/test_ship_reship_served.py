...
class SourcePins(unittest.TestCase):
    def test_payload_retained_and_reshipped_on_the_reconnect_event(self):
        self.assertIn('interface PendingShip { name: string; shipId: string; b64?: string; kind: "composer" | "comment"; queued?: boolean; postSeq?: number }', RENDER)   # + the ship's kind and the shim-queue mark (round twelve)
